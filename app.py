...
SESSIONS_DIR = os.path.join(WATCH_DEFAULT, "Sessions")
COACH_TEXTS_DIR = os.path.join(BASE_DIR, "coach_texts")

# -----------
# Study mode
# -----------
STUDY_MODE = True  # True = User study mode; False = Ollama active

SCENARIOS = {
    "reading": {
        "label": "Reading Session",
        "file": os.path.join(SESSIONS_DIR, "reading.csv"),
        "task": "Reading",  
    },
    "drawing": {
        "label": "Drawing Session",
        "file": os.path.join(SESSIONS_DIR, "drawing.csv"),
        "task": "Drawing",
    },
    "logic_puzzle": {
        "label": "Logic Puzzle Session",
        "file": os.path.join(SESSIONS_DIR, "logic_puzzle.csv"),
        "task": "Logic Puzzle",
    },
    "math": {
        "label": "Math Task",
        "file": os.path.join(SESSIONS_DIR, "math.csv"),
        "task": "Math",
    },
    "game": {
        "label": "Game Session",
        "file": os.path.join(SESSIONS_DIR, "game.csv"),
        "task": "Game",
    },
    "music": {
        "label": "Music Listening Session",
        "file": os.path.join(SESSIONS_DIR, "music.csv"),
        "task": "Music",
    },
    "meditating": {
        "label": "Meditation Session",
        "file": os.path.join(SESSIONS_DIR, "meditating.csv"),
        "task": "Meditating",
    },
}

def load_coach_text_from_file(scenario_id: str, kind: str) -> str | None:
    """
    Load prewritten coach text
    """
    if not scenario_id:
        return None
    filename = f"{scenario_id}_{kind}.html"
    path = os.path.join(COACH_TEXTS_DIR, filename)
    try:
...
def load_scenario_into_state(scenario_id: str):
    ...
    try:
        ...
        df = load_csv(path, cache_buster=mtime)
        st.session_state["df"] = df
        st.session_state["file_name"] = os.path.basename(path)
        st.session_state["scenario_id"] = scenario_id
        st.session_state["task"] = scenario.get("task") or scenario.get("label")
        return True
    except Exception as e:
        st.error(f"Could not load scenario data: {e}")
        return False

def get_study_coach_text(kind: str) -> str | None:
    if not STUDY_MODE:
        return None
    ...
    return load_coach_text_from_file(scenario_id, kind)

def get_current_scenario():
    sid = st.session_state.get("scenario_id")
    return SCENARIOS.get(sid)

def render_start_body_study():
    """
    Center content for the Start page when STUDY_MODE is True:
    - lets user pick one of the 7 prepared sessions
    - moves them to Personalization
    """
    st.markdown(
        "<div class='caption'>When you’re ready, choose an example session or upload your own data.</div>",
        unsafe_allow_html=True,
    )

    st.markdown("#### 1. Choose a prepared session")

    scenario_labels = {sid: info["label"] for sid, info in SCENARIOS.items()}
    label_to_id = {v: k for k, v in scenario_labels.items()}

    chosen_label = st.selectbox(
        "Select one of the example recordings to explore:",
        list(scenario_labels.values()),
        key="scenario_select",
    )
    chosen_id = label_to_id.get(chosen_label)

    use_scenario = st.button("✨ Use this example", key="use_example_button")

    if use_scenario and chosen_id:
        if load_scenario_into_state(chosen_id):

            st.session_state["profile"] = {
                "task": SCENARIOS[chosen_id]["task"],   # preselect scenario task
                "age_range": "",
                "gender": "",
                "occupation": "",
                "eeg_experience": "",
            ...
            st.session_state["task"] = SCENARIOS[chosen_id]["task"]

            st.session_state["page"] = "Personalization"
            st.rerun()

    st.markdown(
        "#### 2. Or upload your own session (optional)",
        unsafe_allow_html=True,
    )

def render_start_body_normal():
    """
    Original behaviour:
    - scan WATCH_DEFAULT for latest CSV
    - show 'Found it' / 'Oh no'
    - Get Results button
    """
    # scanning indicator
    status_ph = st.empty()
    status_ph.markdown(
        "<div class='caption'><span class='loader'></span>Scanning for your latest session…</div>",
        unsafe_allow_html=True,
    )
    ...
    if latest:
        ...
        disabled = False
        msg_html = (
            "<div class='fadein' style='font-size:0.95rem; color:#2E8B57; margin-top:.8rem;'>"
            "✅ <b>Found it!</b></div>"
        )
    else:
        file_name = None
        disabled = True
        msg_html = (
            "<div class='fadein' style='font-size:0.95rem; color:#D97706; margin-top:.8rem;'>"
            "⚠️ <b>Oh no!</b> There's nothing here yet — try saving your session or use the manual upload below."
            "</div>"
        )
    ...
    with btn_c:
        ...
        st.markdown(
            msg_html.replace("margin-top:.8rem;", "margin-top:.8rem; text-align:center;"),
            unsafe_allow_html=True,
        )
...
if page == "Start":
    ...
    with center:
        ...
        st.markdown(
            "<div class='subtitle'>Your session’s complete — let’s explore how your mind performed.</div>",
            unsafe_allow_html=True,
        )
...
if page == "Personalization":
    ...
    SCENARIO_TASK_TO_PRETTY = {
        "reading": "Reading",
        "drawing": "Drawing / Doodling",
    ...
    scenario = get_current_scenario() if STUDY_MODE else None

    if STUDY_MODE and scenario:
        raw = (scenario.get("task", "") or "").lower().strip()
        default_task_value = SCENARIO_TASK_TO_PRETTY.get(raw, "")
        prof["task"] = default_task_value
        st.session_state["task"] = default_task_value
    else:
        default_task_value = prof.get("task", "")
    ...
    if STUDY_MODE and scenario:
        ...
        st.markdown(
            f"<div class='task-pill'>📌 Task: {default_task_value}</div>",
            unsafe_allow_html=True,
        )
    else:
        prof["task"] = st.selectbox(
            ...
            task_opts,
            index=_safe_index(task_opts, default_task_value),
            key=f"task_{nonce}",
...
if page == "Display":
        ...
        if mode == "Visual":
            ...
            if not to_plot:
                ...
                if ai_on:
                    ...
                    if STUDY_MODE:
                        text = get_study_coach_text("visual")
                        if text:
                            st.markdown(
            ...
            if not to_plot:
                ...
                if ai_on:
                    ...
                    if STUDY_MODE:
                        text = get_study_coach_text("literal")

                        if text:
                            st.markdown(
                                f"""
                                <div class='ai-mini'>
                                    <div class='ai-mini-title'>🧘 AI Coach Reflection</div>
                                    {text}
                                </div>
                                """,
                                unsafe_allow_html=True,
            ...
            if STUDY_MODE:
                ...
                if text:
                    st.markdown(
                        ...
                        <div class="llm-coach">
                            ...
                            <div class="llm-bubble">{text}</div>
                        </div>
                        """,
                        unsafe_allow_html=True,
                    )
                else:
                    st.info("No study-mode LLM text found for this scenario.")
                ...
                calm_val        = summary_data.get("calm_score")
                drift_val       = summary_data.get("drift_score")
                focus_val       = summary_data.get("focus_score")
                movement_score  = summary_data.get("movement_score")
    ...
    _, center_btn, _ = st.columns([3, 2, 3])
    with center_btn:
            if st.button("↺ Start over", key="back_to_start_study", type="primary", use_container_width=True):
